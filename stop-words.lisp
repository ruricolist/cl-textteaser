...
(def stop-words
  (set-hash-table
   '("-" " " "" "." "a" "e" "i" "o" "u" "t"
     ...
     "beyond" "both" "bottom" "but" "by" "call" "can" "cannot" "can't" "co" "con"
     "could" "couldn't" "de"
     ...
     "sun" "rappler" "rapplercom" "inquirer" "yahoo" "home" "sports" "sa" "says" "tweet"
     "pm" "home" "homepage" "sports" "section" "newsinfo" "stories" "story" "photo"
     "year" "years" "percent" "time" "january" "february" "march" "april" "may" "june" "july" "august" "september" "october" "november"
     "december" "government" "police")
   :strict nil :test 'equal))

(defun too-short? (w) (< (length w) *min-word-length*))

(defun all-digits? (w) (every #'digit-char-p w))

(defun stop-word? (w)
  (or (too-short? w)
      (all-digits? w)
      (nth-value 1 (@ stop-words w))))

(defun sans-stop-words (words)
  (remove-if #'stop-word? words))
